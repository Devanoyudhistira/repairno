...
export default function Productcard({ image, productname, harga,id }) {
    return <Card size="md" className={`border-primary m-0 cursor-pointer transition-all hover:shadow-lg hover:-translate-y-1 border p-1 gap-0 bg-primary-foreground`} >
        <Image className="w-full h-full mt-1 object-center object-cover" src={"/image/" + image} alt="productname" width={300} height={300} />
        <CardContent className={`px-0 py-1 m-0 flex  h-full flex-col justify-between `} >
            ...
            <CardFooter className={`w-full  px-2 flex items-center justify-between  font-medium`} >
                <h1 className="text-secondary font-mono font-medium text-md" > {convertToMoney(harga)} </h1>
                <Link href={`/shop/${id}`} > <Button size="icon" variant="outline" className={`border-secondary`} > <ArrowUpRight /> </Button> </Link>
            </CardFooter>
        </CardContent>
    </Card>
}