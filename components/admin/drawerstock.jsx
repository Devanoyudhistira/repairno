...
export default function Drawerstock({ condition, setcondition, setaction, children , drawertitle,drawerescription }) {
    const [state, action, pending] = useActionState(setaction, null)
    useEffect(() => {
        if (state?.success) {
    ...
    return <Drawer open={condition} onOpenChange={setcondition} >
        <DrawerContent>
            <DrawerHeader>
                ...
                <DrawerDescription> {drawerescription} </DrawerDescription>
            </DrawerHeader>
            <form action={action} className="w-full bg-primary-foreground h-60 gap-5 items-center flex flex-col" >
                {children}
